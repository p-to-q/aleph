...
| [`kaggle-runbook.md`](kaggle-runbook.md) | Offline recovery of a completed legacy Kaggle run into a canonical detailed receipt, including strict identity, raw-output preservation, scalar replay, no-replace publication, and blocked empty/invalid-usage/near-cap diagnostics. |
| [`kaggle-diagnostic-runbook.md`](kaggle-diagnostic-runbook.md) | A zero-call-first v0.2 Kaggle deployment canary: exact runtime/package gates, fixed six-prompt transport check, detailed receipt, and a hard non-leaderboard boundary. |
| [`kaggle-capture-runbook.md`](kaggle-capture-runbook.md) | One-shot operation of the Python-3.11-compatible raw-output capture canary, with a durable retry barrier, exact-run binding, and a strict no-score publication boundary. |
| [`platform-release-policy.md`](platform-release-policy.md) | The single-public-benchmark policy: stable Kaggle identity, private engineering surfaces, promotion gates, platform readback, and Hugging Face mirroring. |
| [`../research/benchmark-platform-engineering.md`](../research/benchmark-platform-engineering.md) | Two-round review of academic benchmark rigor, platform runtime behavior, evidence layering, and the GitHub/Kaggle/Hugging Face authority split. |
| [`../research/model-release-benchmark-engineering.md`](../research/model-release-benchmark-engineering.md) | Engineering survey of 20 model-release benchmarks, Berkeley/BAIR systems, evaluation harnesses, reporting identity, and Aleph's concrete gaps. |
| [`../plans/kaggle-canonical-numeric-release.md`](../plans/kaggle-canonical-numeric-release.md) | The staged plan for a portable numeric Task, exact paid-run control, private release candidates, same-URL Kaggle promotion, and a matching Hugging Face release. |
| [`../plans/benchmark-release-records-and-kaggle-harness.md`](../plans/benchmark-release-records-and-kaggle-harness.md) | Four-layer release/run/result/event records, the Kaggle runtime harness, cross-surface notes and retention, and the evidence-led implementation sequence. |
| [`../plans/kaggle-capture-only-and-canonical-offline-scoring.md`](../plans/kaggle-capture-only-and-canonical-offline-scoring.md) | Proposed separation of Kaggle raw-output capture from Python 3.13/UCD 15.1 canonical scoring, with staged call budgets, exact-run binding, and publication gates. |
...
- The checked-in v0.1 HF/Kaggle/Croissant delivery package is immutable and `aleph-bench package` is
  ...
  [`PLATFORM_LAUNCH_CHECKLIST.md`](../../bench/results/platform/m0-mock/PLATFORM_LAUNCH_CHECKLIST.md).
