...
| Aleph-Bench M0 is a runnable benchmark pipeline with deterministic mock evidence. | experimental benchmark claim | `bench/`, `schemas/aleph-bench-*.schema.json`, `bench/results/m0-first-run.json`, `docs/benchmark/m0-evidence.md` | Account A for benchmark meaning, Account B for adapter behavior | Do not present the checked-in mock result as a real model leaderboard; hosted black-box rows need a separate run with credentials. |
