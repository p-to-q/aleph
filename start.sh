#!/usr/bin/env bash
# Aleph dev startup — brings up the active frontend and backend servers in tmux panes.
# Usage:  ./start.sh
...
if ! command -v tmux >/dev/null 2>&1; then
  ...
  echo "Active servers starting in the background."
...
# Pane 0 — Active Next.js frontend (port 3000)
...
# ── attach ───────────────────────────────────────────────────────────────────
echo "Active servers starting in tmux session '$SESSION'."
echo "  port 3000  →  Next.js frontend"
echo "  port 8000  →  MLX search server"
echo "  port 8010  →  Aleph API"
echo ""
